...
class Mailer2 < ActionMailer::Base
  ...
  def reminder(user, issues, days, startday=false)
    ...
    if startday 
      mail :to => user.mail,
        :subject => l(:mail_subject_reminder_startday, :count => issues.size, :days => days)
    else
      mail :to => user.mail,
  ...
  def self.reminders(options={})
    ...
    days = options[:days] || 7
    startday = options[:startday] || false
    ...
    issues_by_assignee.each do |assignee, issues|
      reminder(assignee, issues, days, startday).deliver if assignee.is_a?(User) && assignee.active?
    end
  end
  ...
  def mail(headers={}, &block)
    headers.merge! 'X-Mailer' => 'Redmine',
            ...
            'List-Id' => "v2:<#{Setting.mail_from.to_s.gsub('@', '.')}>"
